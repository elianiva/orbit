...
layer(RenderServiceLive)("render", (it) => {
  it.effect("markdown to html with GFM features", () =>
    Effect.gen(function* () {
      ...
      const result = yield* render.toHtml("# Hello\n\n**bold** and `code`");
  ...
  it.effect("markdown with code block gets expressive code treatment", () =>
    Effect.gen(function* () {
      const render = yield* RenderService;
      const result = yield* render.toHtml("```ts\nconst x = 1;\n```");
  ...
  it.effect("mdx with EmbeddedHtml component", () =>
    Effect.gen(function* () {
      const render = yield* RenderService;
      const result = yield* render.toMdx('<EmbeddedHtml html="<span>raw</span>" />');
  ...
  it.effect("malformed mdx returns RenderError", () =>
    Effect.gen(function* () {
      const render = yield* RenderService;
      const result = yield* Effect.exit(render.toMdx("{invalid mdx syntax <<<"));
